...
import { iframeHandlerPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [iframeHandlerPlugin],
    });

    await request(server).get('/api/iframe-handler/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/iframe-handler/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/iframe-handler/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/iframe-handler/todos/${createdTodoItem.id}`)
